...
window = SDL.create_window 0, 0, width, height, 0
#surface = SDL::Surface.new(SDL.get_window_surface(window), width, height, 32)
renderer = SDL::Renderer.new(window)
#surface = SDL.set_video_mode width, height, 32, LibSDL::HWSURFACE | LibSDL::ASYNCBLIT | LibSDL::DOUBLEBUF | LibSDL::FULLSCREEN
  ...
  (height / 10).times do |h|
    (width / 10).times do |w|
      ...
      10.times do |y|
        10.times do |x|
			if rect
				renderer.draw_pixel(x + 10 * w, y + 10 * h, 255, 0, 0) 
			else
				renderer.draw_pixel(x + 10 * w, y + 10 * h, 0, 0, 0)
			end
...
#  surface.unlock
  ...
  # TODO render_present instead of flip
